function [fullfit, foldfits] = cvglmfitsparseprior(y,X,U,folds,varargin)
...
%     
    ...
    %% greatest lambda for which dev = mindev*1.05
    semDev = std(mean(cvdeviances,2))/sqrt(size(cvdeviances,2));
    [minDev, minDevIdx] = min(fullfit.deviances);
    min1sem = minDev+semDev;
    idx = find(fullfit.deviances(1:minDevIdx)<min1sem,1,'first');
