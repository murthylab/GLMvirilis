function [p,a1,a2] = confplot(varargin)
...
pTmp = plot(x,y,x,z1,x,z2);    
YLIM = get(gca,'YLim'); delete(pTmp);

% a1 = fill([x(:)' fliplr(x(:)')],[z1(:)' fliplr(z2(:)')],'r',...
%         'LineStyle','none');
a1 = areashade(x,z1,z2,'r');
a2 = a1;
% a1 = area(x,z1,min(YLIM));
 hold on;
% set(a1,'LineStyle','none'); set(a1,'FaceColor',[1.0 0.6 0.6]);
...
set(gca,'Layer','top');
 ...
 if (nargout>1) varargout{1} = p; varargout{2} = a1; varargout{3} = a2; end;
