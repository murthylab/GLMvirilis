...
if nRows > 1
   ...
   for col = 1:nCols
      ...
      if ~verLessThan('matlab','8.4')% detect new graphics system
         x = handles.bar(col).XData;
      else
         x = get(get(handles(col).bar,'children'),'xdata');
      end
   ...
   hErrorbar = errorbar(mean(x,1), y, lowerErrors, upperErrors, '.k');
