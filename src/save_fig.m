...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	switch(computer)													% Get gs command.
		case 'MACI64',		gs= '/usr/local/bin/gs';
...
%    	gs=		[gs		'  -dUseCIEColor'];			% Probably good.     <-- THIS GAVE A WARNING WITH GS 9.15
	gs=		[gs		' -dAutoRotatePages=/None'];			% Probably good.
	gs=		[gs		' -dHaveTrueTypes'];								% More probably good.
    %gs=     [gs     ' -dSubsetFonts=true -dEmbedAllFonts=true'];
	cmdEnd=			' -sDEVICE=%s -sOutputFile="%s" ';					% Essential.
	epsCmd=			'';
   %epsCmd=	[epsCmd ' -dSubsetFonts=true -dEmbedAllFonts=true'];% Future support?
	epsCmd=	[epsCmd ' -dSubsetFonts=true -dEmbedAllFonts=true -dNOPLATFONTS'];% Future support?
	epsCmd=	[epsCmd ' -dColorConversionStrategy=/UseDeviceIndependentColor' ...
					' -dProcessColorModel=/%s'];						% Supported by gs in future?
	pdfCmd=	[epsCmd cmdEnd '-dPDFSETTINGS=/prepress -c .setpdfwrite'];							% Recommended by gs.
...
%	device.png.cmyk=	sprintf(cmdEnd,	'png???', 					[fname '.png']);
	...
	print(fighandle, deps{:}, '-noui','-loose', renderer, res, [fname '-temp']);	% Output the eps.
